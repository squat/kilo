...
func (t *Topology) Rules(cni, iptablesForwardRule bool) iptables.RuleSet {
	...
	for _, s := range t.serviceCIDRs {
		rules.AppendRules = append(rules.AppendRules, iptables.NewRule(iptables.GetProtocol(s.IP), "nat", "KILO-NAT", "-d", s.String(), "-m", "comment", "--comment", "Kilo: do not NAT packets destined for service CIDRs", "-j", "RETURN"))
	}
